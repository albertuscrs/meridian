...
export const CONFIG_MAP = {
  ...
  lockMaxVolatility: ["screening", "lockMaxVolatility"],
  ...
  r8ExitPreset: ["management", "r8ExitPreset"],
  r8OorCooldownHours: ["management", "r8OorCooldownHours"],
...
// user-config.json accumulates dead keys silently (e.g. maxBundlePct, which was
// never read anywhere) — an unknown key means the operator believes a setting is
// active when it is not. config.js reads startup-only keys directly as `u.<key>`,
// so scan its source instead of maintaining a second hand-written whitelist.
export function findUnknownUserConfigKeys({ userConfigPath = USER_CONFIG_PATH } = {}) {
  const userConfig = readJsonSafe(userConfigPath, {});
  let startupKeys = [];
  try {
    const configSrc = fs.readFileSync(repoPath("config.js"), "utf8");
    startupKeys = [...configSrc.matchAll(/\bu\.([A-Za-z_$][\w$]*)/g)].map((m) => m[1]);
  } catch { /* fail-open: CONFIG_MAP keys still count as known */ }
  const known = new Set([
    ...Object.keys(CONFIG_MAP),
    ...Object.keys(OPERATOR_ONLY_KEYS),
    ...startupKeys,
    "telegramAllowedUserId", // read back by telegram.js
    "preset",                // persisted by setup.js
  ]);
  return Object.keys(userConfig).filter((k) => !k.startsWith("_") && !known.has(k));
}
