 ...
 */
export function confirmPeak(position_address, candidatePnlPct, confirmTicks = 2, options = {}) {
...
// ─── Backward-compat shims (intentional local edit) ──────────────────────────
// These were kept in the local fork for the 15s-setTimeout recheck path that
// index.js uses in its management cycle safety net. Upstream removed them in
// favor of the unified 2-tick registerExitSignal/confirmPeak above. We keep
// them so index.js's queuePeakConfirmation / scheduleTrailingDropConfirmation
// helpers still resolve without rewiring the LLM-driven path.
export function queuePeakConfirmation(position_address, candidatePnlPct, options = {}) {
  ...
  if (options.immediate) {
    ...
    pos.peak_pnl_pct = Math.max(currentPeak, candidatePnlPct);
    save(state);
    return true;
  }
  // Default: defer to the 2-tick confirmPeak path so callers don't need to know
  // about both APIs.
  return confirmPeak(position_address, candidatePnlPct, 2);
}
...
export function resolvePendingTrailingDrop(position_address, currentPnlPct, trailingDropPct, tolerancePct = 1.0, windowLabel = "15s") {
  ...
  const pendingDrop = pos.pending_trailing_drop_pct ?? 0;
  const currentDrop = pendingPeak - currentPnlPct;
  const recovered = currentDrop <= pendingDrop - tolerancePct;
  if (recovered) {
    pos.pending_trailing_current_pnl_pct = null;
    ...
    save(state);
    log("state", `Position ${position_address} trailing drop recovered within ${windowLabel} window`);
    return { confirmed: false, recovered: true };
  }
  pos.pending_trailing_current_pnl_pct = null;
  pos.pending_trailing_peak_pnl_pct = null;
  pos.pending_trailing_drop_pct = null;
